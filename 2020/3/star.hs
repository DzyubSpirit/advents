...
numTreesOnSlope :: TreeMap -> (Int, Int) -> Int
numTreesOnSlope treeMap (dy, dx) = length $ filter (== '#') $ map (treeMap !)
                                 $ takeWhile ((< height treeMap) . fst)
                                 $ iterate (bimap (+ dy) (+ dx)) (0, 0)
...
main = do
  ...
  print $ product $ map (numTreesOnSlope treeMap) 
    [(1, 1), (1, 3), (1, 5), (1, 7), (2, 1)]
